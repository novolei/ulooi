...
- M0.5 真机结论已进入实现：FEDA handshake、FED0 30ms `.withoutResponse` heartbeat、FED1 pitch、FED2 brightness、FED8 4s battery poll、FED9 telemetry subscription。M1.2 DevTools 真机修正：FED1 `0xFF` 是 nod-down auto-return，保持低头用 `0xB0`；FED2 app-level full 使用 `0xFE`，避开不可靠的 `0xFF`。
